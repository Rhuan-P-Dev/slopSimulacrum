...
| `data/worldObjects.json` | World objects (props) placed in rooms — each names a blueprint (so its matter is real, validated data), a room, and a room-relative position. Materialized at boot as `isStatic` entities (so they can be hit, broken, and harvested); the `isStatic` flag is what keeps them out of the turn roster; see [World Objects](subMDs/systems/world_objects.md) |
...
| `data/world_rules.json` | World-rules layer — stable string key → small config objects governing cross-cutting laws (not loot tables); ships the `damageTornMaterial` rule (a deterministic X% of applied loss drops as torn matter per composition fraction), the `onDamage` event law (a small per-event chance to drop a chunk of the damaged component's material, additive to the per-material chunk drops). The `energyFlow` rule (the cross-component energy circulation law: per-turn share of turn-start energy plus the default per-component capacity bound — see [Energy Flow](subMDs/systems/energy_flow.md)) was **removed from the shipped data** (energy off by data); the code path remains and would pick the rule back up on boot if the key is re-added; a missing/empty/malformed file means all rules and event laws off (legacy world, never crashes) |
| `data/crafting.json` | Crafting recipe definitions (inputs/outputs referencing inventory item types; inputs may additionally be the self-describing dynamic `chunk_<material>` types, which have no registry entry by design) — ships the two knife→T1 assemblies and the two coal-conversion recipes (`t1_to_coal`, `wood_chunk_to_coal`) |
