...
| 2 | Agent tick wiring | [`src/server.js`](src/server.js:71) | The turn system's NPC-agent hook (fires once per round at local tick `TURN_NPC_AGENT_TICK = 20`) already routes deterministic-brain entities to `npcAIController.think(npcEntityId, round, entity)` and everything else to the LLM agent. **No `server.js` change needed.** |
...
| 12 | Turn system | [`src/controllers/core/TurnSystemController.js`](src/controllers/core/TurnSystemController.js), [`src/utils/Constants.js`](src/utils/Constants.js:100) | Round = 360 ticks; planning window = ticks [0, 300); NPC agent tick = 20; max 3 queued actions per entity per round. The drone queues **at most one** action per round (its single decision). |
...
plus a new private method `craftLoopBehavior` on the class. No constructor-injection change (the behavior uses only the per-tick snapshot args and the already-injected `facade`).

### 4.2 Constants (new, in `src/utils/npcAiUtils.js`)

```js
export const PICK_RANGE = 100;            // keep in sync with PickUpItemHandler.maxRange and dropItem range in data/actions.json
export const TARGET_ITEM_TYPE = 'knife';  // data-driven fallback; the behavior prefers the recipe input type
export const RECIPE_ID = 'single_knife_to_t1';
export const CRAFT_OUTPUT_TYPE = 't1';
```

(These mirror the existing `ATTACK_RANGE`/`PUNCH_ACTION`/`MOVE_ACTION` pattern in `npcAiUtils.js`. If desired, the behavior may resolve the target type at runtime from `facade.getCraftingRecipes()[RECIPE_ID].inputs[0].type` and fall back to `TARGET_ITEM_TYPE` when the recipe is absent — recommended, keeps the recipe as the single source of truth.)

### 4.3 Decision flow (stateless — re-derived from the facade every tick)
...
craft_loop(ctx):
  ...
  if dist <= PICK_RANGE:
      ...
      return null        # pickup committed this tick; the craft happens in Stage A next round
...
stateDiagram-vr
    ...
    Idle --> Approaching : dropped knife found beyond PICK_RANGE
    Approaching --> Approaching : knife still beyond PICK_RANGE -> move action each round
    Approaching --> PickedUp : knife within PICK_RANGE -> executePickUpItem immediate
    ...
    PickedUp --> Idle : pickup rejected by handler (stale/range) -> re-derive next round
```
...
| 2 | dist 140 | `move` | — |
| 3–15 | dist decreases by ≤ 10 per round until ≤ 100 | `move` | — |
| 16 | dist ≤ 100 | `null` (idle) | `executePickUpItem(drone, knife, core)` → knife on `crafterCore` |
| 17 | holds knife | `dropItem { itemId: newT1, itemType: 't1', targetX/Y: drone pos }` | `craftItems(drone, 'single_knife_to_t1', core, [knife.id])` → T1 on core |
| 18 | T1 on the ground at drone's position | `null` or next-knife pursuit | — |
...
| 5 | [`src/utils/npcAiUtils.js`](src/utils/npcAiUtils.js) | Export `PICK_RANGE` (= 100), `RECIPE_ID` (= `'single_knife_to_t1'`), `TARGET_ITEM_TYPE` (= `'knife'`), `CRAFT_OUTPUT_TYPE` (= `'t1'`), each with a "keep in sync with …" comment. Export a pure helper `findNearestDroppedItem(droppedItems, roomId, x, y)`: min Euclidean distance with lexicographic-id tie-break; the helper owns the room + finite-coordinate guards and returns `{ item, distance }` or `null`. The `PICK_RANGE` pickup decision is owned by the behavior (mirrors `chase_attack`), not the helper — no range parameter. |
| 6 | [`src/controllers/ai/NpcAIController.js`](src/controllers/ai/NpcAIController.js) | (a) Register `'craft_loop'` in the `_behaviors` Map in the constructor. (b) Add private method `_craftLoopBehavior(ctx)` implementing the §4.3 flow exactly (Stage A → B → C), using only the listed facade public APIs: `getCraftingRecipes`, `getEntityItems`, `craftItems`, `getDroppedItems`, `executePickUpItem`, plus `entity.location/spatial/components` from the passed snapshot. No direct access to `facade.*Controller` internals, no reads of `facade._*` fields. |
...
Fixture: a drone entity (`isNPC: true`, `npcConfig.ai.behavior = 'craft_loop'`, blueprint `crafterDrone`, `location: 'room-main'`, `spatial: {x:0, y:0}`, components `crafterCore`/`crafterArm`/`crafterRollingBall` with typed ids) + a mock facade exposing `getEntity`, `getEntities`, `getActionRegistry` (with `move` and `dropItem` entries), `canEntityExecuteAction`, `executeAction`, `getDroppedItems`, `getEntityItems`, `craftItems`, `executePickUpItem`, `getCraftingRecipes`, and a mock `turnSystem` (planning open / closed variants), mirroring the existing test file's `makeFacade`.
...
4. **Pick up — knife within range**: knife at distance 50 (≤ 100) → `executePickUpItem(droneId, knifeDroppedId, coreCompId)` called exactly once with the core component's id; `think()` returns idle (no action queued this round).
5. **Pick up — boundary**: knife at exactly distance 100 → still picks up (`≤` semantics, mirroring the handler's `> maxRange` rejection).
...
2. **Full cycle end-to-end**: (a) place a `knife` on the ground in `start_room` away from the drone (e.g., have a player entity execute `dropItem` on a held knife, or dispatch the drop handler directly); (b) advance the tick system through ≥ 3 full rounds (planning + agent tick 20 + resolution); (c) assert: drone's `spatial` moved toward the knife's coordinates; the knife is gone from `getDroppedItems()` and present in the drone's `getEntityItems()`; the drone no longer holds the knife but holds a `t1`; `getDroppedItems()` contains a new entry with `itemType === 't1'` at (or within `move`-distance of) the drone's position; the dropped T1's `ownerId === drone.id`.
...
- [`test/unit/npcAiUtils.test.js`](test/unit/npcAiUtils.test.js) — add cases for the new exported constants/helper (`findNearestDroppedItem` distance ordering, id tie-break, empty-input guard) if the helper is added.
...
- **R2 — Crafting on a turn**: the crafting wiki states crafting is a UI-panel, non-turn-cost operation and that NPC brains are "invisible to crafting". **Decision: the drone triggers `facade.craftItems(...)` directly during its brain tick (free, like the panel), and consumes its one turn action for the `dropItem` of the result. No new registry action is added** (adding one would contradict the documented design; the wiki prescribes registering a real action only if a round cost is ever wanted).
...
- **R9 — Pick-up threshold**: `PICK_RANGE = 100` matches the hardcoded `maxRange` in `PickUpItemHandler` (which mirrors `dropItem`'s range in `data/actions.json`; the client's pickup preview uses the same symmetric value). Named constant in `npcAiUtils.js` with a sync comment, following the existing `ATTACK_RANGE` precedent. The drone stops issuing `move` at `≤ 100`, so the handler's re-validation at pick time always passes for a stationary knife.
- **R10 — Concurrency with other agents**: a player (or LLM NPC) could steal the target knife between rounds, or the drone's `executePickUpItem` could fail at call time. The behavior is stateless and re-derives from the facade every round, so it self-corrects; handler failures are structured results (logged, non-throwing) and never break the turn loop (root guard in `think()`).
