...
## 7. Turn State in the Aggregated View & Persistence Schema

The aggregated state exposes a `turns` section: round number, current tick, phase, initiative-ordered actor list, per-entity action queues, and an additive `barrier` sub-state (round-start roster, ready set, close tick and reason). The phase is stored — not merely derived — because the planning window closes on an event (all-ready), not on the tick clock, and the barrier sub-state persists for the same reason: a restore mid-planning must resume the same round's barrier (same roster, same remaining planners) rather than silently re-planning the round. The persistence schema is strictly versioned (v3, with the barrier section) and old-version snapshots are rejected by the tested version contract — saves are operator tooling, not user data.
