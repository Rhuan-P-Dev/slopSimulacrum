...
| **World Objects (Static Props)** | Static-Entity Props (no dedicated controller) | World objects from `data/worldObjects.json` are materialized at boot as **`isStatic` entities** — spawned by the root facade's `_spawnStaticProps()` into the ordinary entity map (no separate store). Being real entities, they are targetable/breakable/harvestable through the normal combat pipeline (a punch drains the component's existence; at 0 it breaks, drops material chunks, and despawns). Being `isStatic`, `TurnSystemController` excludes them from the turn roster so they never gate the all-ready planning barrier; they also opt out of the player loadout and serialize/restore like any other entity — see [World Objects](../systems/world_objects.md) |
...
├── CraftingPanel (new)
├── KnowledgePanel (new)
├── OverlayManager (replaces ConfigBarManager)
├── PickUpOverlayController
└── GroupPickOverlayController (cluster pickup window)
```
