...
On success, the channel-damage step writes a reserved entry into the consequence context's action params (shape: the target ID and the **applied** loss — the existence delta that actually left the target, clamped to what the target had). The dispatcher's existing propagation (`propagateParams: true` on the single-attacker path, see [`ConsequenceDispatcher._dispatchConsequences`](src/controllers/consequences/ConsequenceDispatcher.js:367)) carries it to later consequences in the same pipeline — the same mechanism `consumeItemAndDamage` already uses to hand `itemVolume` to the damage step. On the multi-attacker path the same entry is carried forward *only inside each attacker's own isolated context* (D11, revised), so every drop step sees exactly the loss its own damage step published — never another attacker's.
...
4. (No change required.) `WorldStateController.getItemStats` does not look up an item definition — it reads the item instance's already-stored `traits`, which `InventoryManager.addItem` populated from the synthesized 100%-single-material definition at add-time (site 2). Chunk items therefore already report material-derived stats with no modification here. (The earlier draft listed it as a synthesis site; that premise was incorrect.)
...
### D11 — Multi-attacker punches: per-fist independent chunk drops (per-attacker loss publication)

On the multi-attacker path ([`executeMultiAttacker`](src/controllers/consequences/ConsequenceDispatcher.js:102)), the dispatcher (a) passes unknown consequence types through per attacker unchanged — so `dropMaterialChunk` runs once per attacker — and (b) carries forward exactly one reserved value: the applied loss each attacker's own damage step publishes. Because every attacker is dispatched into an **isolated** context, a loss published by one fist is visible only to that fist's own later consequences and never crosses attacker boundaries. The drop handler therefore performs **one independent roll per fist**, each computed from that fist's own applied loss and that fist's own attacking material — two fists, two independent chunks, never one combined chunk (BUG-134).

**Why per-attacker publication is not the aggregation the path avoids.** The multi-attacker path deliberately never merges handler output *across* attackers — no single number may stand for what two independent attacks did together, because that would make later steps depend on dispatch order and hide which attack caused them. But a chunk is a record of *lost matter*, and each attacker's damage is an independent event against the target, so the matter each fist tears off is independent too. Carrying the reserved loss key forward inside each attacker's *own* isolated context is therefore not an aggregate: it is the single-attacker contract, applied per attacker. The drop still consumes only a published loss and still recomputes nothing; the publication rule simply changes from "propagate everything" (single-attacker path) to "propagate only the reserved loss key, and only inside this attacker's context" (multi-attacker path). Every *other* handler-modified param keeps the no-propagation semantics, so the damage math, the synergy scaling, and the formula-exact loss numbers are untouched.
...
- **Multi-attacker punch:** each fist deals (synergy-scaled) channel damage and drops *its own* chunks — one independent roll per fist, each from that fist's own applied loss and attacking material; the two fists' chunks are never combined and no aggregate loss is ever formed (D11, revised).
...
4. *No drop without damage:* a punch whose value resolves to 0 (on either path) → no chunk records; on the multi-attacker path a fist drops only if that fist's own damage step published a positive loss.
