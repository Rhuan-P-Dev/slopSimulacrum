...
**Status:** Approved design — implementation-ready for the Code sub-task.
...
```mermaid
flowchart TD
    A[tick fires - job energy-flow - order 2] --> B{flow enabled at boot?}
...
`initialize()` does three things at boot, in order: (1) resolves the `energyFlow` rule config once (rules are static per world — the data files are only ever loaded at boot — so a boot-time read is both sufficient and cheaper than a per-tick read); (2) fail-soft-validates the optional per-recipe `energyCapacity` fields in the injected registry (present-but-malformed → warn once per recipe and ignore the field, falling back to the rule default — never a boot failure, matching the world-rules degradation discipline); (3) registers the job and logs **one** `Logger.info()` line (enabled: the share, the default capacity, the recipe-override count, the job id/interval/order; or disabled: the reason, "legacy behavior — no flow").
...
Runtime behavior when off: the tick job remains registered (stable id) and its callback returns immediately — no stat work, no entity enumeration, no log line per tick.
...
- `beginEnergyFlowTick()` — increments a private flow-scope counter (initialized 0, mirroring the cascade counter field).
- `endEnergyFlowTick(shouldBroadcast)` — decrements; when the counter returns to 0, the facade performs **exactly one** full-state broadcast via the wired broadcast service, but only when `shouldBroadcast` is true *and* a broadcast service is wired (unwired → silent no-op, as in hand-built test worlds).
...
| Flow disabled at boot (rule missing/malformed/off-by-design) | Job callback returns immediately — no enumeration, no writes, no per-tick log | one boot-time line per the §4 table |
| Broadcast service unwired (test worlds) | `endEnergyFlowTick` no-ops the broadcast silently; the flow step itself is unaffected | silent |
...
| `src/controllers/WorldStateController.js` | Constructor: accept and store the energy-flow controller (field, **not** in the broadcast `subControllers` map — no owned state); call its `initialize()` right after the turn system's. Add the flow-scope counter field + `beginEnergyFlowTick()` / `endEnergyFlowTick(shouldBroadcast)`; extend the component stat-change broadcast gate with the flow-scope conjunct. |
...
- **(d) Coexistence with the coal generator — charge then flow.** Drive the IC job (`'internal-components'`, order 0) and then the flow job at the same tick, as the tick system executes them in order. Assert the values equal the charge-then-flow math: the +10 burn (tick multiple of 5, coal available from the world.json kit) is present in the flow's tick-start read, so the body's post-tick value reflects flow applied to the *post-charge* energy, and the other 22 components receive a share of the post-charge value.
...
| 2 | Tick job | id `energy-flow`, interval 1, order 2, registered in `initialize()` | Order 1 (ties with the turn system — execution order would then depend on stable-sort registration order, an unmodeled coupling); a sub-tick interval (the mechanic is per-tick by design; coarser cadence changes the convergence contract). |
...
| 9 | Broadcast mitigation | Facade flow batch scope (counter + one conjunct in the existing gate + begin/end pair) → exactly one full-state broadcast per flow tick, and only if a write occurred | Nothing (up to N full-state packets per tick); a throttle inside the broadcast service (global behavior change for all callers, still emits when nothing changed); batching inside the component controller (wrong layer — a state store must not know about clients). |
...
| 12 | Degradation posture | Missing/malformed → flow fully off, job stays registered as a no-op, warn once at boot, never crash, never per-tick log | Throwing at boot (a balance file becoming a crash vector — the rules layer's explicit rejection of that); per-tick "rule missing" warns (log spam from a legitimate configuration). |
