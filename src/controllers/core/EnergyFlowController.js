 ...
 * interconnected (body-like). Each turn, every component sends a fixed
 ...
 * remaining capacity and overflow is lost for that turn (never re-routed).
 ...
 * retired, so this controller carries no tick-system dependency.
 *
 ...
 *   - broadcast discipline via the facade's beginEnergyFlowTurn() /
 *     endEnergyFlowTurn(shouldBroadcast) scope pair (exactly one full-state
 *     broadcast per flow turn, and none when nothing moved).
 *
 * Dependency shape (constructor-based root, one-way flow, public API only):
 * constructed by the composition root with two named dependencies —
 * (1) componentRegistry (data/components.json, used ONLY to resolve the
 * optional per-recipe `energyCapacity` bound — never stat values, which the
 * recipe model forbids), (2) worldRulesController (the flow reads the
 ...
 * pattern both peer controllers use.
 *
 ...
 * boot-time read is sufficient and cheaper than a per-turn read). When the
 * rule is inactive the turn step returns immediately — no stat work, no
 * enumeration, no per-turn log line. Failure isolation is the composition
 * root's per-subsystem guarded block (L2); this controller additionally
 * wraps each step in begin → work → finally close so the broadcast scope
 * always closes (a partially-written step then broadcasts exactly once with
 * the partial state, the correct representation of what actually happened).
 *
 ...
 * write — and broadcast — every turn at the uniform point).
 * @type {number}
 */
...
class EnergyFlowController {
     ...
     */
    constructor(componentRegistry, worldRulesController) {
     ...
     * a flow turn then degrades to a silent no-op.
     ...
     *       validation, no warn);
     *   (3) logs exactly ONE info line.
     * The flow registers no tick job: its per-turn step is driven from the
     * turn system's round-start hook (see processFlowTurn). Side-effect-free
     * until a round actually starts.
     * @returns {void}
     */
    initialize() {
        ...
        // (3) One init line (enabled or disabled). No tick job is registered:
        // the flow runs once per turn, driven from the turn system's round-start
        // hook (see processFlowTurn).
        if (this._rule) {
            Logger.info(
                ...
                `one redistribution per turn, after the IC turn step)`
            );
        } else {
            Logger.info(
    ...
    /**
     * One flow turn: the simultaneous read-all / compute / write-all pass over
     * every live entity, executed ONCE PER TURN (driven from the turn system's
     * round-start hook, strictly after the IC turn step — charge-then-flow).
     * Each entity is one fully-interconnected network with N ≥ 2 components
     * (fewer → no-op, no division by zero possible); an entity whose turn-start
     * energy sum is 0 is skipped entirely (zero-energy entities do no work).
     * Sends are computed from turn-start values ONLY (simultaneity: no
     * component's write feeds another component's send in the same turn), each
     * send is divided equally among the other N−1 components, and each
     * component's new value is clamped to [0, its capacity] — overflow is lost
     * for the turn, never re-routed. A component is written (SET, damage event
     * suppressed) only when its clamped value meaningfully differs from its
     * turn-start value (the FLOW_NOOP_EPSILON no-op rule: steady-state and
     * zero-energy worlds perform zero writes and zero broadcasts per turn).
     *
     * The `round` argument is used for log consistency only — the step runs
     * every turn, including round 0, and is not gated on the round number.
     *
     ...
     * once with the partial state. Failure isolation beyond that is the
     * composition root's per-subsystem guarded block (the flow step is guarded
     * independently, so an IC-step failure can never skip it).
    ...
    processFlowTurn() {
        ...
        try {
            ...
            for (const entity of allEntities) {
                ...
                // READ-ALL: turn-start energy of every component. A missing or
                ...
                // A zero-energy entity does no work (spec: skip when the SUM of turn-start
                ...
                // COMPUTE (simultaneous step — turn-start values only): send(c) =
                // share × start(c), split equally among the other N−1 components, clamped
                // to [0, capacity] — overflow lost for the turn.
                ...
                // from the turn-start value. SET semantics so unseeded
                ...
                // would otherwise drop chunks on every circulated turn).
                for (let i = 0; i < n; i++) {
            ...
            // full-state broadcast per flow turn, and only if something moved.
            world.endEnergyFlowTurn(anyWrite);
        }
    }

    /**
     * Reads a component's turn-start Physical.energy through the facade's
     ...
     * @returns {number} The turn-start energy (0 for absent/non-numeric).
     * @private
     */
     ...
     * @param {number[]} starts - Turn-start energies, indexed per component (all ≥ 0).
    ...
    _computeEntityNextValues(starts, components, share) {
        ...
        for (let o = 0; o < n; o++) {
            ...
            // capacity — overflow is LOST for this turn.
