...
describe('Player initial-spawn loadout (real world, data-driven)', () => {
    ...
    it('applies the full data/world.json loadout to the incarnated player with no "no valid slot" warnings', () => {
        ...
        // metalBox — a firstFit:centralBall,droidArm,droidHand container — with its
        // 5 nested knife children (children added via addItemToContainer).
        const metalBox = items.find(i => i.type === 'metalBox');
        expect(metalBox).toBeDefined();
        const knivesInMetalBox = items.filter(i => i.type === 'knife' && i.hostComponentId === metalBox.id);
        expect(knivesInMetalBox.length).toBe(5);

        // testItem / testItem2 (both droidArm), and t1 (bestAvailable:hand → a hand).
        const testItem = items.find(i => i.type === 'testItem');
        const testItem2 = items.find(i => i.type === 'testItem2');
        ...
        // The standalone top-level knife (not the ones nested inside metalBox or t1).
        const topLevelKnife = items.find(
            i => i.type === 'knife' && i.hostComponentId !== metalBox.id && i.hostComponentId !== t1.id
        );
        expect(topLevelKnife).toBeDefined();

        // Full expected count: metalBox(1) + knivesInMetalBox(5) + testItem(1)
        // + testItem2(1) + standalone knife(1) + t1(1) + t1 ammo(1) = 11.
