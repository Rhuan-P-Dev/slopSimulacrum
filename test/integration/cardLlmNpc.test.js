...
describe('data/cards → M1 LLM NPCs (random room, per-card identity)', () => {
    ...
    it('spawns one distinct entity per card PNG (m1Droid, isNPC, card display name)', () => {
    ...
    it('routes cards (no registry entry) through the per-entity config, not a shared blueprint', () => {
    ...
    it('renders each card\'s name + personality in the LLM system prompt', () => {
        const npcs = agent.getNpcs();
        const frisk = npcs.find((n) => n.entity?.name === 'Frisk');
        expect(frisk).toBeDefined();
